...
def ejecutar_sql(sql: str) -> tuple[list[list], list[str]]:
    ...
    sentencias = [
        s.strip()
        for s in '\n'.join(ln for ln in sql.splitlines() if not ln.lstrip().startswith('--')).split(';')
        if s.strip()
    ]
    errores = []
...
def ejecutar_prolog() -> dict[str, list[list]]:
    if SWIPL is None:
        sys.exit('falta swipl (SWI-Prolog 9.x) en el PATH')
    main = DIR / '_main_verificar.pl'
    main.write_text(PROLOG_MAIN, encoding='utf-8')
    try:
        out = subprocess.run(  # noqa: S603
            [SWIPL, '-q', '-g', 'main', '-t', 'halt', str(DIR / 'consultas.pl'), str(main)],
            capture_output=True,
            text=True,
            encoding='utf-8',
            cwd=DIR,
