...
Como se mostró en los capítulos 1 y 4, `2 + 3` es un término compuesto, no un
...
El operador `=`, usado desde el capítulo 1, es la **unificación**: pregunta si
...
operador, `==`, que se presenta en el capítulo 10.
...
expresión tengan valor, y opera en un único sentido.
...
actividad de la sección 8.2, con `luis =:= luis`. En el segundo, la expresión
...
El contraste con `suma/3` del capítulo 6, definida sobre los términos
`s(s(0))`, es ilustrativo. Aquella admitía consultas en ambos sentidos, porque
...
prácticos que la notación `s(s(s(0)))`. El capítulo 32 presenta una técnica que
...
`s(s(0))`, la unificación con la estructura decidía sola qué cláusula
...
Por eso, en un predicado que usa aritmética conviene anotar en el comentario
**qué argumentos deben tener valor** al invocarlo. No es una formalidad: es la
diferencia entre un predicado que se puede consultar en cualquier sentido y uno
que solo es correcto sobre cierto conjunto de consultas.
...
verifica cada una. Es la plantilla 5 del capítulo 3, y se desarrolla en detalle
en el capítulo 9.
...
`largo/2` del capítulo 7 y `generaciones/3` del capítulo 6.

Esa forma tiene una limitación, señalada al final de la sección 7.5: mientras la
...
```prolog
% suma_lista(L, S): S es la suma de los números de L.
...
```prolog
% suma_con_acumulador(L, S): la misma relación, con un acumulador.
suma_con_acumulador(L, S) :-
    sumando(L, 0, S).

% sumando(L, Hasta, Total): Total es Hasta más la suma de los elementos de L.
...
disponibles. Esta diferencia tiene consecuencias: el capítulo 14 explica por qué
...
```prolog
% dar_vuelta(L, R): R es L en orden inverso. En este caso el acumulador no es un
% número: es la lista que se construye.
dar_vuelta(L, R) :-
    dando_vuelta(L, [], R).
...
La versión del capítulo 7 usaba `append/3` para agregar cada elemento al final.
...
su costo, y no por sus respuestas. El capítulo 14 trata ese tema.
...
4. **(2)** Escribir `es_par(N)` para los enteros predefinidos, con `mod`.
5. **(2)** Escribir `mayor_de_los_dos(A, B, M)` sin usar `max`.
6. **(2)** Escribir `cuantos_mayores(L, N)`: N es la cantidad de números de la
   lista `L` mayores que 18. Usar la plantilla 9.
...
12. ★ **(2)** El predicado siguiente pretende contar de 1 a `N`, pero no termina.
    ...
    Conviene tener presente lo que dice la sección 8.3 sobre lo que se pierde al
    pasar de `s(s(0))` a los números predefinidos.
...
| Tema | Se retoma en |
|---|---|
| Generar y probar, con el corte para evitar trabajo repetido | capítulo 9 |
| `==` y `\==`, similares a `=` y a `=:=` pero distintos de ambos | capítulo 10 |
| Por qué el acumulador usa menos memoria | capítulo 14 |
| Sumar o contar sin escribir la recursión | capítulos 15 y 16 |
| Aritmética que admite consultas en ambos sentidos | capítulo 32 |
