...
Dos, tres y cuatro `s`, respectivamente. El `0` interior no se cuenta: es el
...
`natural(perro)`. `perro` no unifica con `0` ni con `s(N)`, de modo que ninguna
...
<!-- ejemplo: capitulo-06/soluciones.pl predicado: mayor/2 consulta: mayor(s(s(0)), s(0)). -->
```prolog
...
condición adicional: al eliminar niveles de dos números iguales se llega a `0` y
`0` simultáneamente, y en ese punto ninguna de las dos cláusulas es aplicable.

## 4

<!-- ejemplo: capitulo-06/soluciones.pl predicado: doble_natural/2 consulta: doble_natural(s(s(0)), D). -->
```prolog
...
<!-- ejemplo: capitulo-06/soluciones.pl predicado: par/1 consulta: par(s(s(0))). -->
```prolog
...
`par(s(0))` responde `false.` de manera directa: `s(0)` no unifica con `0` ni
...
capítulo 7 generaliza con la plantilla 12.
...
a. ```prolog
   ...
   hasta que el primero es `0`; ahí el caso base establece que el resultado es
...
b. ```prolog
   ...
   `s`, y los quita de los dos a la vez; cuando el primero llega a `0`, el caso
...
- `valor(s(s(0)), V).` funciona y responde `V = 2`. La recursión avanza sobre
  el primer argumento, que está instanciado y se reduce en cada llamada.
- `valor(N, 2).` **no termina**. Con `N` libre, la segunda cláusula construye
  `s(N1)`, después `s(s(N2))`, y así de manera indefinida: nada se reduce. El
  objetivo `V is Anterior + 1` no puede corregir el rumbo, porque se evalúa
  recién **después** de la llamada recursiva.
- `valor(s(N), 3).` **tampoco termina**, y por la misma razón: `s(N)` fija el
  primer nivel, pero `N` sigue libre, y a partir de ahí el crecimiento es el
  mismo.

La lección es la de la sección 6.5: la terminación depende de qué argumentos
llegan instanciados. `valor/2` solo es utilizable en un sentido, y el comentario
del predicado debería decirlo.

## 13

<!-- ejemplo: capitulo-06/soluciones.pl predicado: menor_o_igual/2 consulta: menor_o_igual(A, s(0)). -->
```prolog
...
variables del capítulo 4.

Es el caso base `menor_o_igual(0, _)` el que produce ese `_`: no exige nada del
segundo argumento.

## 14

<!-- ejemplo: capitulo-06/soluciones.pl predicado: impar/1 paridad/2 consulta: paridad(s(s(0)), P). -->
```prolog
...
plantilla 4 del capítulo 3: la relación se cumple por un caso o por el otro.

`paridad(s(0), par).` responde `false.`, y el trabajo previo es el que conviene
observar: Prolog prueba la primera cláusula, que lo lleva a `par(s(0))`, y esa
consulta recorre la definición de `par/1` sin encontrar cláusula aplicable
—`s(0)` no unifica con `0` ni con `s(s(N))`—. Recién entonces descarta la
...
Es decir: la respuesta es inmediata, pero no gratuita. El capítulo 9 muestra
